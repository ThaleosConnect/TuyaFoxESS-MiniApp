/**
 * CRC-16 (MODBUS) calculation utility
 */
 ...
 * @return {number} - The calculated CRC value
 */
export function crc16modbus(data) {
  let crc = 0xFFFF;
  
  for (let i = 0; i < data.length; i++) {
    const pos = (crc ^ data[i]) & 0xFF;
...
/**
 * Verify that the CRC in the given data is correct
 * 
 * @param {Uint8Array} data - The data including the CRC (last 2 bytes)
...
export function verifyCrc(data) {
  ...
  const messageData = data.slice(0, data.length - 2);
  const expectedCrc = crc16modbus(messageData);
